...
import { deleteUserFromEntity } from '~/api/entityService';
...
function UserList() {
  ...
  const totalPages = Math.ceil((memberList?.length - 1) / usersPerPage);
  ...
  return (
    <>
              ...
              <div key={user.email} className="contents">
                ...
                <div className="flex items-center gap-5">
                  ...
                  <ActionButton
                    type="delete"
                    onClick={async () => {
                      const res = await deleteUserFromEntity(token, user.email, userInfo.legalEntityCode);
      ...
      <div className="flex mt-6 justify-center">
        <Pagination totalPages={totalPages} searchParams={searchParams} setSearchParams={setSearchParams} />
      </div>
    </>
  );
}
